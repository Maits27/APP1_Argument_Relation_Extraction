...
params_file_path = "/gaueko1/users/mmartin/ENVIRONMENT/NLIsrc/params/train_params.json"
...
def main():
    ...
    def compute_metrics_viejo(
            eval_predictions
    ):
        logits, references = eval_predictions
        predictions = np.argmax(logits, axis=-1)
        results = {}
        results['eval_micro_f1'] = f1.compute(predictions=predictions, references=references,average='micro')['f1']
        results['eval_macro_f1'] = f1.compute(predictions=predictions, references=references, average='macro')['f1']

        results['eval_micro_precision'] = \
            precision.compute(predictions=predictions, references=references, average='micro')['precision']
        results['eval_macro_precision'] = \
            precision.compute(predictions=predictions, references=references, average='macro')['precision']
        results['eval_micro_recall'] = recall.compute(predictions=predictions, references=references, average='micro')[
            'recall']
        results['eval_macro_recall'] = recall.compute(predictions=predictions, references=references, average='macro')[
            'recall']
        results['eval_accuracy'] = accuracy.compute(predictions=predictions, references=references)['accuracy']
        return results
    ...
    def compute_metrics(eval_pred):
        ...
        return {
            ...
            'f1_ent': class_f1['f1'][0],
            'f1_neu': class_f1['f1'][1],
            'f1_con': class_f1['f1'][2],
            ...
            'precision_ent': class_precision['precision'][0],
            'precision_neu': class_precision['precision'][1],
            'precision_con': class_precision['precision'][2],
            'recall_micro': recall_value_micro['recall'],
            'recall_macro': recall_value_macro['recall'],
            'recall_weighted': recall_value_weighted['recall'],
            'recall_ent': class_recall['recall'][0],
            'recall_neu': class_recall['recall'][1],
            'recall_con': class_recall['recall'][2],
