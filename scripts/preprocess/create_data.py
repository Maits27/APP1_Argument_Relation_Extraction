...
def conseguir_datos_al_completo_paso1(ficheros, initial_data_path, split_char = '\t', split_info = ' '):
    ...
    for f in ficheros:
        ...
        with open(f'/gaueko1/users/murruela002/APP1/NLIsrc/{f}_data.json', "w", encoding='utf-8') as archivo:
...
def conseguir_relaciones_paso2(ficheros, a_train, a_dev, a_test, split_char = '\t', split_info = ' '):
    ...
    for f in ficheros:
        ...
        datos_json['files'] = len(datos_json['data'])
        # with open(f'/gaueko1/users/murruela002/APP1/NLIsrc/Preproceso/Anotation/Relation_Data/{f}_relations.json', "w", encoding='utf-8') as archivo:
        #     json.dump(datos_json, archivo, ensure_ascii=False, indent=4)
        if f == 'train':
            train = datos_json
        elif f == 'dev':
...
def entrenamiento_a_base_de_opciones_paso4(train_dev_test, neutrals=3, lang='en'):
    ...
    for f_type, data in train_dev_test:
        ...
        # with open(f'/gaueko1/users/murruela002/APP1/NLIsrc/Datasets/Dataset_sin_preguntas2/{all_file_name}.json',
        #         'w') as archivo_json:
